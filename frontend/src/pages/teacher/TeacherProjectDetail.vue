...
<template>
  ...
  <div v-if="project && !loading" class="page teacher-project-detail-page">
    ...
    <div class="demo-teacher-detail-layout">
      ...
      <aside class="demo-teacher-detail-aside"><section class="paper-card"><h2>指导动作</h2><div class="demo-action-stack"><RouterLink class="primary-button" :to="teacherReviewRoute(undefined, project.id)">查看待审核材料</RouterLink><RouterLink class="secondary-button" :to="{ path: '/teacher/ai', query: { projectId: String(project.id), mode: 'research' } }">进入 AI 指导室</RouterLink><RouterLink class="secondary-button" :to="`/teacher/projects/${project.id}/template`">配置材料范本</RouterLink><RouterLink class="secondary-button" :to="teacherMembersRoute(project.id)">成员与邀请</RouterLink><button v-if="project.status === 'completed' && (!publicCase || ['rejected', 'pending_teacher'].includes(publicCase.status))" class="secondary-button" type="button" :disabled="inviting" @click="inviteConfirm = true">{{ inviting ? '正在发出邀请…' : '邀请全平台展示' }}</button><div v-else-if="publicCase" class="case-invite-state"><StatusTag :status="publicCase.status" /><small>{{ publicCase.status === 'waiting_student' ? '等待项目负责人同意' : publicCase.status === 'pending_platform' ? '等待平台审核' : publicCase.status === 'published' ? '已进入全平台案例库' : '当前申请未进入可发起状态' }}</small></div></div></section><section class="paper-card"><h2>指导提醒</h2><div class="callout"><strong>{{ risk }}</strong><span>{{ summary.needsReview ? `当前有 ${summary.needsReview} 项材料需要处理，建议优先查看最早提交的版本。` : '当前没有需要教师立即介入的风险。' }}</span></div></section></aside>
    </div>
  </div>
