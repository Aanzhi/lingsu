...
const selectedProject = computed(
  () =>
    teacherStore.state.guided.find(
      (item) => item.id === Number(route.params.id),
    ) ?? teacherStore.state.guided[0],
);
...
const titles = computed<Record<string, [string, string, string]>>(() => ({
  ...
  projects: ["指导项目", "项目进度与风险", "查看团队、阶段和材料进度。"],
  project: [
    "指导项目",
    selectedProject.value?.title ?? "项目详情",
    "项目团队与真实成长数据。",
  ],
  reviews: [
    "审核收件箱",
    "学生材料审核",
    "查看正文、附件和版本，给出清晰可执行的反馈。",
  ],
  review: [
    "材料审核",
...
async function handleFeedbackAction() {
  ...
  await teacherStore.load().catch((reason) => {
    feedback.value = makeFeedback(
      "error",
      errorMessage(reason),
      "数据没有刷新成功，请重试。",
  ...
  <div class="page teacher-page">
    ...
    <template v-if="surface === 'home'"
      ><div class="metric-grid teacher-metrics">
        <RouterLink to="/teacher/reviews" class="metric-card risk"
          ><small>待审核材料</small
          ><strong>{{ teacherStore.state.reviews.length }}</strong
          ><span>需要处理</span><el-icon><ArrowRight /></el-icon></RouterLink
        ><RouterLink to="/teacher/pool" class="metric-card"
          ><small>待认领项目</small
          ><strong>{{ teacherStore.state.pool.length }}</strong
          ><span>来自本校学生</span
          ><el-icon><ArrowRight /></el-icon></RouterLink
        ><RouterLink to="/teacher/members" class="metric-card"
          ><small>成员待确认</small
          ><strong>{{ teacherStore.state.invitations.length }}</strong
          ><span>确认后进入团队</span
          ><el-icon><ArrowRight /></el-icon></RouterLink
        ><RouterLink to="/teacher/projects" class="metric-card"
          ><small>指导项目</small
          ><strong>{{ teacherStore.state.guided.length }}</strong
          ><span
            >{{
              teacherStore.state.guided.filter(
          ...
          ><el-icon><ArrowRight /></el-icon></RouterLink
        ><button
          class="metric-card metric-card--archived"
          type="button"
          @click="navigateTab('archived')"
        >
          <small>已归档</small
          ><strong>{{ teacherStore.state.archived.length }}</strong
          ><span>已完成项目</span><el-icon><ArrowRight /></el-icon></button
        ><button
          class="metric-card metric-card--trashed"
          type="button"
          @click="navigateTab('trashed')"
        >
          <small>回收站</small
          ><strong>{{ teacherStore.state.trashed.length }}</strong
          ><span>30 天自动清除</span><el-icon><ArrowRight /></el-icon>
        </button>
      </div>
      <section class="desk-panel">
        <div class="section-heading">
          ...
          <RouterLink to="/teacher/reviews">查看全部 →</RouterLink>
        </div>
        <RouterLink
          v-for="revision in teacherStore.state.reviews.slice(0, 4)"
          :key="revision.id"
          :to="`/teacher/reviews/${revision.id}`"
          class="review-row"
          ><span class="file-glyph">V</span>
          <div>
            <strong>{{ revision.material_title }}</strong
            ><small
              >{{ revision.project_title }} · {{ revision.author_name }}</small
            >
          </div>
          <StatusTag :status="revision.status" /><span
            >开始审核 →</span
          ></RouterLink
        ><EmptyState
          v-if="!teacherStore.state.reviews.length"
          title="收件箱已清空"
          description="当前没有需要判断的提交；新的材料提交后会自动进入这里。"
    ...
    ><template v-else-if="surface === 'pool'"
      ><div class="pool-grid">
        <article
          v-for="project in teacherStore.state.pool"
          :key="project.id"
          class="pool-card paper-card"
        >
          <header>
            <StatusTag :status="project.status" /><span>{{
          ...
          <footer>
            <span
              ...
              >{{ project.members[0]?.username }}</span
            ><button
        ...
        <EmptyState
          v-if="!teacherStore.state.pool.length"
          title="当前没有待认领项目"
          description="学生新建项目后会自动出现在本校项目池。"
          ><RouterLink class="secondary-button" to="/teacher/projects"
            >查看指导项目</RouterLink
          ></EmptyState
        >
      </div></template
    ><template v-else-if="surface === 'projects' || surface === 'project'">
      <nav
        v-if="surface === 'projects'"
        class="project-lifecycle-tabs"
        role="tablist"
      >
        <a
          href="#"
          :class="{ 'is-active': activeTab === 'guided' }"
          role="tab"
          @click.prevent="navigateTab('guided')"
          >指导中 <span>{{ teacherStore.state.guided.length }}</span></a
        >
        <a
          href="#"
          :class="{ 'is-active': activeTab === 'archived' }"
          role="tab"
          @click.prevent="navigateTab('archived')"
          >已归档 <span>{{ teacherStore.state.archived.length }}</span></a
        >
        <a
          href="#"
          :class="{ 'is-active': activeTab === 'trashed' }"
          role="tab"
          @click.prevent="navigateTab('trashed')"
          >回收站 <span>{{ teacherStore.state.trashed.length }}</span></a
        >
      </nav>

      <div v-if="surface === 'project' && selectedProject" class="guided-grid">
        <RouterLink
          :to="`/teacher/projects/${selectedProject.id}`"
          class="guided-card guided-card--detail"
        >
          <div class="section-heading">
            <StatusTag :status="selectedProject.status" /><span>{{
          ...
          <div class="project-health">
            ...
            <div>
              <small>成长值</small
              ><strong>{{ selectedProject.growth.experience }} XP</strong>
            </div>
            <div>
              <small>等级</small
              ><strong>Lv.{{ selectedProject.growth.level }}</strong>
            </div>
          </div>
          <footer>
            <span>{{
              selectedProject.members.map((item) => item.username).join("、")
            }}</span
            ><b>查看项目 →</b>
          </footer>
        </RouterLink>
      ...
      <div v-else-if="activeTab === 'guided'" class="guided-grid">
        <article
          v-for="project in teacherStore.state.guided"
          :key="project.id"
          class="guided-card"
        >
          <RouterLink
            :to="`/teacher/projects/${project.id}`"
            class="guided-card-link"
          >
            <div class="section-heading">
              <StatusTag :status="project.status" /><span>{{
                project.project_type
            ...
            <div class="project-health">
              <div>
                <small>成员</small><strong>{{ project.members.length }}</strong>
              </div>
              <div>
                <small>成长值</small
                ><strong>{{ project.growth.experience }} XP</strong>
              </div>
              <div>
                <small>等级</small
                ><strong>Lv.{{ project.growth.level }}</strong>
              </div>
            </div>
            <footer>
              <span>{{
                project.members.map((item) => item.username).join("、")
              }}</span
              ><b>查看项目 →</b>
            </footer>
          </RouterLink>
        ...
        <EmptyState
          v-if="!teacherStore.state.guided.length"
          title="还没有指导项目"
          description="认领学生项目后，可在这里跟进团队、任务和材料进度。"
          ><RouterLink class="secondary-button" to="/teacher/pool"
            >前往项目池</RouterLink
          ></EmptyState
        >
      </div>
      ...
      <div v-else-if="activeTab === 'archived'" class="guided-grid">
        <article
          v-for="project in teacherStore.state.archived"
          :key="project.id"
          class="guided-card guided-card--archived"
        >
          <RouterLink
            :to="`/teacher/projects/${project.id}`"
            class="guided-card-link"
        ...
        <EmptyState
          v-if="!teacherStore.state.archived.length"
          title="没有已归档的项目"
          description="完成的项目可以归档保存，保持项目书架整洁。"
        />
      </div>

      <div v-else-if="activeTab === 'trashed'" class="guided-grid">
        <article
          v-for="project in teacherStore.state.trashed"
          :key="project.id"
          class="guided-card guided-card--trashed"
        >
          <div class="guided-card-link">
            ...
            <div class="project-health">
              ...
              <div>
                <small>剩余</small
                ><strong>{{ project.days_until_purge ?? 30 }} 天</strong>
              </div>
            </div>
            <footer>
              <span
                >移入回收站于
                {{ project.trashed_at?.slice(0, 10) ?? "--" }}</span
              >
            </footer>
          </div>
          <div class="guided-card__actions" @click.stop>
        ...
        <EmptyState
          v-if="!teacherStore.state.trashed.length"
          title="回收站是空的"
          description="删除的项目会在这里保留 30 天，逾期后会被自动清除。"
        />
      </div> </template
    ><template v-else-if="surface === 'reviews'"
      ><section class="review-inbox">
        <div class="inbox-list">
          <RouterLink
            v-for="revision in teacherStore.state.reviews"
            :key="revision.id"
            :to="`/teacher/reviews/${revision.id}`"
            class="inbox-item"
            ><span class="file-glyph">V</span>
          ...
          ><EmptyState
            v-if="!teacherStore.state.reviews.length"
            title="没有待审核材料"
            description="学生提交后将保留版本、真实性确认与附件检查状态。"
          />
        </div>
    ...
    ><template v-else-if="surface === 'members'"
      ><section class="member-queue paper-card">
        <div
          v-for="invite in teacherStore.state.invitations"
          :key="invite.id"
          class="member-approval"
        >
          <span class="avatar soft">{{ invite.invitee_name.slice(0, 1) }}</span>
          <div>
            <strong>{{ invite.invitee_name }}</strong
            ><small>申请加入：{{ invite.project_title }}</small>
          </div>
          <StatusTag status="waiting_teacher" /><button
        ...
        <EmptyState
          v-if="!teacherStore.state.invitations.length"
          title="没有待确认成员"
