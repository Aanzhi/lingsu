  ...
  <div v-if="task && project && material" class="page task-page">
    ...
    <div class="task-layout demo-task-layout">
      <section class="task-response-card demo-task-main task-main-column paper-card" :class="{ 'task-paper--read-only': !canEdit }">
        ...
        <div v-else-if="material.status === 'submitted'" class="submitted-state"><el-icon><Clock /></el-icon><div><strong>已提交，等待教师审核</strong><p>你可以离开此页面；审核结果会通过工作通知和研究进程同步。</p><div v-if="body" class="submitted-copy"><small>已提交版本</small><p>{{ body }}</p></div></div></div>
