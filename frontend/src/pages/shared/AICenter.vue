...
async function saveRename() {
  if (!current.value || !titleDraft.value.trim()) return
  try {
...
<template>
  <div class="conversation-page">
    ...
    <main class="chat-main">
      <header class="chat-header"><div><span class="eyebrow">全局 AI 工作台</span><div v-if="renaming" class="rename-row"><input v-model="titleDraft" aria-label="对话标题" @keydown.enter="void saveRename()" /><button type="button" @click="void saveRename()">保存</button><button type="button" @click="renaming = false">取消</button></div><div v-else class="title-row"><h1>{{ current?.title || '新建科创对话' }}</h1><button v-if="current && !current.is_archived" class="rename-button" type="button" @click="startRename">重命名对话</button></div><small>{{ currentProject?.title || '未绑定项目 · 通用咨询' }}</small></div><div class="chat-actions"><button type="button" @click="agentOpen = !agentOpen">Agent{{ currentAgent ? ` · ${currentAgent.name}` : '' }}⌄</button><button type="button" @click="contextOpen = !contextOpen">上下文{{ contextOpen ? '收起' : '展开' }}</button><button v-if="current && !current.is_archived" type="button" @click="archiveCurrent">归档</button></div></header>
